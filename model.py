...
def get_outputs(inputs, skip=False, is_training=True, keep_rate=0.75):
    ...
    with tf.variable_scope("Model", reuse=tf.AUTO_REUSE) as scope:
        ...
        #fake_images_b=  current_decoderb(tf.concat([latent_a,latent_b_diff], axis=3), tf.expand_dims(images_b[:, :, :, 1], axis=3), name='de_A', skip=skip)
        #fake_images_a = current_decodera(tf.concat([latent_b,latent_a_diff], axis=3), tf.expand_dims(images_b[:, :, :, 1], axis=3), name='de_B', skip=skip)
        ...
        #cycle_images_b=  current_decoderb(tf.concat([latent_fake_a,latent_b_diff], axis=3), fake_images_a, 'de_A', skip=skip)
        #cycle_images_a = current_decodera(tf.concat([latent_fake_b,latent_a_diff], axis=3), fake_images_b, 'de_B', skip=skip)
        
        cycle_images_tmp_b=  current_decoderc(tf.concat([latent_fake_a,latent_b_diff], axis=3), fake_images_a, 'de_c', skip=skip)
        cycle_images_tmp_a = current_decoderc(tf.concat([latent_fake_b,latent_a_diff], axis=3), fake_images_b, 'de_c', skip=skip)
        ...
        # prob_fea_real_a_is_real = current_discriminator(pred_mask_real_a, name="d_F")
    ...
    return {
        ...
        # 'prob_fea_real_a_is_real': prob_fea_real_a_is_real,
...
def build_encoderc(inputen, name='encoder', skip=False, is_training=True, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        k1 = 3
        padding = "CONSTANT"
...
def hw_flatten(x) :
    # return tf.reshape(x, shape=[x.get_shape().as_list()[0], -1, x.get_shape().as_list()[-1]])
...
def build_encoders(o_r12, name='encoder', skip=False, is_training=True, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        k1 = 3
        padding = "CONSTANT"
...
def build_encodert(o_r12, name='encoder', skip=False, is_training=True, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        k1 = 3
        padding = "CONSTANT"
...
def build_encoderdiffa(inputen, name='encoder', skip=False, is_training=True, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 8
        k1 = 3
        padding = "CONSTANT"
...
def build_encoderdiffb(inputen, name='encoder', skip=False, is_training=True, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 8
        k1 = 3
        padding = "CONSTANT"
...
def build_decoderc(inputde, inputimg, name='decoder', skip=False):
    with tf.variable_scope(name):
        f = 7
        ks = 3
        padding = "CONSTANT"
