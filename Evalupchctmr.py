...
import  model620 as model  #model256
import nibabel as nib
import cv2

from stats_func import *
from eval import eval3d, result2DT3D
...
def main(config_filename):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        sess.run(init)
        modellist=['./output/20200705-093821/pch-21599','./output/20200705-093821/pch-21899']
        allresult = open('./result/ctmrtest/dice.txt', 'a+')
        for i in modellist:
            arr=[]
            ...
            for i in range(len(Lines)):
                ...
                arr.append(dice)
                savedir=save_DIR+'/ctmrtest/'+ modelname.split('/')[3]
                if not os.path.isdir(savedir):
                    os.makedirs(savedir)
                dicefile=save_DIR+'/ctmrtest/'+ modelname.split('/')[3]+'/result.txt'
                reultev = open(dicefile, 'a+')
                savename = save_DIR+'/ctmrtest/'+ modelname.split('/')[3]+'/'+ laLines[i] + '.nii.gz'
